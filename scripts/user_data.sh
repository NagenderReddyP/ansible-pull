...
[ ! -f $CLI ] && AWS_CLI_FILE="/opt/awscli.zip" && curl -s "https://awscli.amazonaws.com/awscli-exe-linux-x86_64.zip" -o "$AWS_CLI_FILE" && unzip -q "$AWS_CLI_FILE" && sh ./aws/install && rm "$AWS_CLI_FILE"
...
aws s3 cp s3://creport-${Env}-s3-mgt/ansible/ansible-pull-main.zip  /tmp

#rm -rf /tmp/ansible-pull-main
#cd /tmp && unzip ansible-pull-main.zip && rm -f ansible-pull-main.zip


# execute bootstrap.shsu - root -c "time sh /tmp/ansible-pull-main/scripts/bootstrap.sh | tee /tmp/bootstrap.log 2>&1 "